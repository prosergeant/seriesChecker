...
func (r *Resolver) Resolve(ctx context.Context, kinopoiskID int, isSerial bool) (*Result, error) {
	...
	// Вытаскиваем финальный URL, все kinobox iframe src и полный HTML страницы
	var finalURL string
	...
	log.Printf("resolver: found %d iframes, finalURL=%s, html=%d bytes", len(iframeSrcs), finalURL, len(pageHTML))
